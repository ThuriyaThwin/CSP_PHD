...
\begin{proof}
	Equation~(\ref{eqn:mcts-probound-blnk-hoeffding}) follows from the
	observation that if $i\ne\alpha$, $\overline X_i^{n_i+N}>A$
	if and only if the mean $\overline X_i^N$ of $N$ samples from $n_i+1$
	to $n_i+N$ is at least $A+(A-\overline
	X_i^{n_i})\frac {n_i} N$.
...
\begin{align}
\Lambda_i^b\le&\int\limits_{\overline X_\alpha}^{\overline X_\alpha^{n_\alpha}+\frac N {N+n_i}(1-\overline X_\alpha^{n_\alpha})}2\exp\left(-\varphi(x-\overline X_i^{n_i})n_i\right)dx\nonumber\\
...
however, of the general formalism in Section~\ref{sec:mcts-optimal}.} 
...
by the empirical evaluation (Section~\ref{sec:mcts-empirical-evaluation}),
