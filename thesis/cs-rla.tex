...
%Later on in this paper we discuss several important implementation details, %which further enhance $\lazyastar$.
...
by \lazyastar~when $f_{max}(n)$ is the best $f$-value in \OPEN. %However, different
% actions are taken while the node is in \OPEN.
...
Table~\ref{tbl:rational-lazy-a-star} presents the results averaged
