...
Intelligence. Search problems can be found in navigation, routing,
planning, and numerous other combinatorial domains.  Despite the
...
others in all cases. Similarly, many algorithms have tunable
parameters that need optimizing. The nature of the required solution
(e.g., optimal, suboptimal etc.) also has significant impact on the
desired search algorithm. Selecting the best algorithm and parameters
for the task at hand is thus a non-trivial and important
issue. Moreover, every problem instance induces a search graph, which
can be extremely non-uniform and have areas with radically different
behavior. This calls for algorithms which change their settings
dynamically during the search, so as to better adapt to the task at
hand.
...
probability distribution), and intractability of the model (typically, higher
than that of the search problem itself!).  As a result, this theory
has seen relatively little application to real search problems to date.
