...
VOI estimations of the rational recomputing algorithm decreases. Let $\eta$ be
the \textit{intensity of VOI estimations}---the ratio between the expected
number of VOI estimations of the rational 
recomputing algorithm $N_r$ to the number of VOI estimations of the original
algorithm $N$: $\eta=\IE\left(\frac {N_r} N\right)$.
...
measurement cost $c$ is constant, the expected VOI age over all measurements and all
steps of the algorithm is $c/\eta$.
...
(\ref{eq:rational-w}), the number of VOI estimations $N_r$ of the rational
recomputing algorithm decreases with the logarithm of the computation
